...
public class DickESP extends ESPModule {
    //Gorda. WILL REDO XD
    ...
    public void onRender(Render3DEvent event, Color color) {
        for (final Object o : mc.theWorld.loadedEntityList) {
            if (o instanceof EntityPlayer) {
                ...
                final double n = player.lastTickPosX + (player.posX - player.lastTickPosX) * event.partialTicks;
                final double x = n - mc.getRenderManager().viewerPosX;
                final double n2 = player.lastTickPosY + (player.posY - player.lastTickPosY) * event.partialTicks;
                final double y = n2 - mc.getRenderManager().viewerPosY;
                final double n3 = player.lastTickPosZ + (player.posZ - player.lastTickPosZ) * event.partialTicks;
